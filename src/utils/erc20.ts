...
import MatatakiNFT from "../constants/abi/MatatakiNFT.json";
...
export const getContractMatatakiNFT = (provider: provider, address: string) => {
  console.log('provider', provider)
  console.log('address', address)
  const web3 = new Web3(provider)
  const contract = new web3.eth.Contract(
    (MatatakiNFT as unknown) as AbiItem,
    address,
  )
  return contract
}
